...
## 📍 Current Phase
**Phase 12 — Streamlit dashboard.** Phases 1–11 complete: two sources (HubSpot CRM + billing) flow RAW→STAGING→INTERMEDIATE→MARTS through 7 marts with clustering on date-queried facts; 103 dbt tests + 49 pytest tests all green; `_metrics.yml` + `docs/metrics_glossary.md` catalog 16 metrics with JD glossary; `dbt docs generate` produces the lineage site; schema drift detector guards 37 tracked HubSpot properties; Reverse ETL pushes health metrics back to HubSpot (round trip verified); `--weekly` seeder implemented with state-file-commit-back. **All 3 GitHub Actions workflows live**: `pipeline.yml` (daily 06:00 UTC), `weekly_seed.yml` (Mondays 07:00 UTC), `schema_drift.yml` (daily 05:30 UTC + on PR). 12 GitHub Secrets configured; schema_drift workflow run validated end-to-end. Remaining: 12 (Streamlit), 13 (cost), 14 (README).
...
- [x] 3.4 `--weekly` flag implemented. `seed/generate_mock_data.py::generate_weekly_batch(week_num, existing_companies, pipeline_stages)` produces a deterministic delta (3 companies, 8 contacts, 8 deals, 15 line items) with RNG seeded `SEED + week_num` — each week reproducible, distinct across weeks. IDs prefixed `c_w{n}_*`, `p_w{n}_*`, `d_w{n}_*` so they don't collide with the initial seed. `seed/seed_hubspot.py::run_weekly_seed()` POSTs via the existing idempotent `seed_batch`/`seed_associations` helpers and bumps `state["_weekly_run_count"]` on completion. New contacts attach to a mix of new + existing companies (expansion/renewal scenario). New deals bias toward closed (~50%, mostly won) so MRR visibly grows week-over-week. Live tested: run #1 created 3+8+8+15+16 records in HubSpot cleanly. Deliberate MVP scope cuts (documented in `run_weekly_seed` docstring): does NOT generate lifecycle events for new contacts, does NOT advance existing open deals' stages, does NOT regenerate billing (separate step in weekly_seed.yml workflow).
...
- [x] 9.1 `extract/schema_drift.py` — single script with two CLI modes: `--snapshot` (writes baseline) and `--check` (diffs live vs baseline; exits 0=no drift, 1=BREAKING, 2=non-breaking). Scope is tight: only the 37 properties in `extract.ENTITY_CONFIG` (the contract our pipeline actually reads), not all 200+ HubSpot props per object. Reuses `hubspot_client.get_properties()` (new method; calls `GET /crm/v3/properties/{objectType}`). Tracks `type` and `fieldType` per property — changes to either are breaking; everything else (labels, group names) deliberately ignored. Pretty diff report shows `[OK]/[FAIL]/[WARN]` per object with `REMOVED`/`CHANGED`/`ADDED` lines.
- [x] 9.2 `infra/expected_schema.json` — committed baseline of 37 tracked properties across companies (9), contacts (8), deals (8), line_items (7), products (5). Generated via `python -m extract.schema_drift --snapshot`. Re-snapshot after deliberately accepting a HubSpot schema change. `--check` against fresh baseline confirms zero drift.
- [x] 9.3 `tests/test_schema_drift.py` — 19 pytest tests for the diff logic (no HubSpot API required; mocks via `MagicMock` + `monkeypatch`). Coverage: `normalize()` filters tracked properties and strips unused metadata (label/group/displayOrder); `diff_schemas()` detects removed, type-changed, fieldType-changed, and added properties across multiple object types; `has_breaking_changes()` distinguishes removed/changed (BREAKING) from added-only (informational); `write_baseline`/`load_baseline` round-trip via `tmp_path` and surface a clear "snapshot first" error on missing baseline; `fetch_live_schema()` integration test mocks the HubSpot client and verifies it filters to ENTITY_CONFIG properties only. Full suite now 30/30 (was 11; +19 from 9.3). Runs in <2s.
...
- [x] 10.3 `tests/test_reverse_etl.py` — 19 pytest tests (no live HubSpot/Snowflake; mocks via `MagicMock` + `patch` of `time.sleep`). Required a tiny refactor of `push_to_hubspot.py`: extracted `rows_to_inputs(rows, synced_at_ms)` and `post_batch_with_retry(session, batch)` as pure functions out of `main()`, making each independently testable. Coverage: `chunked()` edge cases (empty/exact/partial); `rows_to_inputs` shape + numeric coercion (Decimal→str) + epoch-ms-as-int regression guard; `post_batch_with_retry` happy path (200/201/207 all succeed), 429 with `Retry-After` header + default 10s fallback, retry within budget (429→429→200), retry budget exhausted (3×429 raises), non-429 4xx fails immediately, 5xx fails immediately (deliberate trade-off — daily cron catches it; tighter than extract.py's 5xx-with-backoff but documented in the test name), endpoint URL + payload shape regression guard. Full suite now **49/49** (was 30; +19 from 10.3). Runs in <1.5s.

**Phase 11 — GitHub Actions**
- [x] 11.1 `.github/workflows/pipeline.yml` — daily 06:00 UTC + `workflow_dispatch` trigger. Single sequential job on `ubuntu-latest`: checkout → setup Python 3.12 (pip cached) → install deps → copy `profiles.yml.example` → profiles.yml (template uses `env_var()` for all credentials so a plain copy works in CI) → `dbt source freshness` (catches a silently-broken prior-day extract) → `python -m extract.extract` → `dbt build` (run + test together) → `python -m reverse_etl.push_to_hubspot` → on failure: curl-POST to Telegram with the run URL. Per-role Snowflake creds injected as env vars (LOADER for extract, TRANSFORMER for dbt, REPORTER for reverse ETL — least privilege preserved in CI). Concurrency group `daily-pipeline` with `cancel-in-progress: false`. Day-1 caveat documented in the file header (freshness check needs RAW already loaded — bootstrap via `workflow_dispatch` or local run). YAML validated (1 job, 9 steps).
- [x] 11.2 `.github/workflows/weekly_seed.yml` — Mondays 07:00 UTC + `workflow_dispatch`. Calls `python -m seed.seed_hubspot --weekly` (implemented in 3.4), commits the updated `seed/.hubspot_ids.json` back to the repo via `github-actions[bot]` so subsequent runs share state. Required un-gitignoring `seed/.hubspot_ids.json` (now committed; the workflow's commit-back step keeps it current). Job has `permissions: contents: write` for the push. Concurrency group `weekly-seed` with `cancel-in-progress: false`. Telegram failure notification same pattern as pipeline.yml. The next morning's daily pipeline picks up the new HubSpot records via extract.extract → regenerate billing → load → dbt build. YAML validated (1 job, 6 steps).
- [x] 11.3 `.github/workflows/schema_drift.yml` — daily 05:30 UTC (intentionally before pipeline.yml's 06:00) + on every PR + `workflow_dispatch`. Runs `extract.schema_drift --check` and maps the three-valued exit code into Actions UX: 0→green, 1→red+telegram, 2→green-with-`::warning::` annotation (non-breaking new property; review and re-snapshot if relevant). Telegram only fires on `schedule` or `workflow_dispatch` runs — PRs surface failure via the PR's checks UI without spamming the team chat. Concurrency group keyed by `${{ github.ref }}` with `cancel-in-progress: true` so PR pushes supersede stale runs. YAML validated (1 job, 5 steps).
- [x] 11.4 GitHub Secrets configured via `gh secret set -f .env` — all 12 required (HUBSPOT_SERVICE_KEY, SNOWFLAKE_ACCOUNT/WAREHOUSE/DATABASE, per-role LOADER/TRANSFORMER/REPORTER user+password pairs, TELEGRAM_BOT_TOKEN/CHAT_ID) plus any incidental .env extras GitHub ignores. End-to-end auth verified via manual `gh workflow run schema_drift.yml` — completed green in 37s on first try.
- [x] 11.5 Telegram failure notifications wired into all three workflows via the standard curl-to-`api.telegram.org/bot{TOKEN}/sendMessage` pattern (matches the user's existing EDGE bot wire-up). Each notification includes the run URL so a tap-through goes straight to the failing log. `schema_drift.yml`'s notification deliberately skips PR runs (PR checks already surface failure in the UI; no need to spam the team chat).

**Phase 12 — Streamlit dashboard**
- [x] 12.1 `dashboard/streamlit_app.py` — connects as `REVOPS_REPORTER` via `st.secrets` (read-only on MARTS; Snowflake enforces — any non-mart query fails at the warehouse). One connection per session via `@st.cache_resource`, results cached 1h via `@st.cache_data`. Hard rule honored throughout: SUM/COUNT/MEDIAN aggregations of mart columns for display are fine; no metric redefinitions in pandas. Local config: `.streamlit/secrets.toml` (gitignored) + `.streamlit/config.toml` (light theme, tracked). Template `.streamlit/secrets.toml.example` committed.
- [x] 12.2 All 8 chart sections from CLAUDE.md spec: KPI strip (Active MRR / Active ARR / Open Pipeline / Win Rate), MRR-by-month bar, ACV histogram, TCV by deal type, Open deals by stage + raw-vs-weighted-pipeline comparison, marketing funnel (counts + per-stage conversion rates), median MQL→SQL days, account health score histogram, top-10 accounts by ARR table. Layout uses `st.columns` for side-by-side cards, native plotly charts (no heavy UI framework). Live tested against Snowflake — all queries return data, no errors.
