...
## 📍 Current Phase
**Phase 11 — GitHub Actions orchestration.** Phases 1–10 complete: two sources (HubSpot CRM + billing) flow RAW→STAGING→INTERMEDIATE→MARTS through 7 marts (dim_accounts, dim_contacts, fct_deals, fct_pipeline, fct_revenue, fct_funnel, fct_account_health) with clustering on the date-queried facts; 103 dbt tests green (87 generic + 13 source freshness/PK + 3 singular business-invariant); `_metrics.yml` catalogs 16 metrics + JD glossary; `docs/metrics_glossary.md` is the stakeholder-facing mirror; `dbt docs generate` produces the lineage site; schema drift detector guards 37 tracked HubSpot properties; Reverse ETL pushes health metrics back to HubSpot (round trip verified). **49 pytest tests green** across extract / schema_drift / reverse_etl. Remaining: 11 (GitHub Actions), 12 (Streamlit), 13 (cost), 14 (README).
